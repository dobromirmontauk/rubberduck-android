...
/**
 * Minimal RIFF/WAVE chunk walker for [FileAudioSource] -- walks chunks by
 * their declared size rather than assuming a fixed header layout, because
 * some encoders (e.g. afconvert, used to generate the WAV fixtures under
 * `app/src/integrationTest/resources/` this feeds on) emit an extra filler
 * chunk between "fmt " and "data" for alignment. Not a
 * general-purpose WAV parser -- just enough to pull PCM samples out of a
 * well-formed 16-bit PCM WAVE file.
 */
object WavDecoder {
    data class Result(val sampleRateHz: Int, val channelCount: Int, val bitsPerSample: Int, val pcm: ByteArray)

    fun decode(input: InputStream): Result {
        ...
        return Result(sampleRateHz, channelCount, bitsPerSample, pcm)
    }
