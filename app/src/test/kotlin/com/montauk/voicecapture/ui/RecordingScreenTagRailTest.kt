 ...
 *
 * Bead asn-3sm: the rail no longer renders always-visible above the
 * transcript pane -- it moved into the double-tap debug view alongside the
 * rest of this screen's pre-asn-3sm chrome (the duck view's thought cloud
 * is now the primary, always-visible tag surface). [setContent] double-taps
 * [DUCK_TRANSCRIPT_TOGGLE_TEST_TAG] right after composing so every test
 * below still exercises the rail directly, unchanged otherwise.
 */
...
class RecordingScreenTagRailTest {
    ...
    private fun setContent(
        ...
        // Bead asn-3sm: the rail lives in the debug view now -- see the class KDoc.
